...
const BtnActiveDoor = () => {
  ...
  return (
    <div className="flex justify-between items-center h-[44px] bg-[white] px-[20px]">
      <marquee className="w-[200px]">
        Nhà hàng Vua Gà tươi chi nhánh Vĩnh Tuy, Trâu Quỳ.
      </marquee>
      {statusOpenDoor && statusOpenDoor ? (
        <div className="flex gap-[8px] justify-center items-center">
          <p className="size-[8px] rounded-[50%] bg-[#16a34a]"></p>
          <p className="text-[8px] font-bold">Đang mở cửa</p>
        </div>
      ) : (
        <div className="flex gap-[8px] justify-center items-center">
          <p className="size-[8px] rounded-[50%] bg-[#dc2626]"></p>
          <p className="text-[8px] font-bold">Đã đóng cửa</p>
        </div>
      )}
    </div>
  );
